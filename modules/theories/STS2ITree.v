...
Section PROOF.
  ...
  Let L1_itree := compile_itree STS_itree.
 ...
 **)
  ...
  Proof.
    ...
    - eapply sim_indC_fin; eauto.
      ...
      unfold ModuleInternal.state_sort. ss.
    ...
    - eapply sim_indC_demonic_tgt; ss; clarify. i.
      ...
      + i. exists (decompile_STS step state_sort st_tgt1).
        ...
        { rewrite unfold_decompile_STS in *. rewrite SRT in *.
          apply (step_choose _ (fun st1 : {st' | step st0 None st'} => decompile_STS step state_sort (st1 $)) (exist _ st_tgt1 STEP)). }
        esplits; et. mbase2.
    - econs; ss.
    ...
    - assert (CASE: (forall ev st1, not (step st0 (Some ev) st1)) \/ (exists ev st1, (step st0 (Some ev) st1))).
      ...
      + set (cont := fun x_ : {ev' : event | exists st1, step st0 (Some ev') st1} =>
        ...
        { rewrite unfold_decompile_STS. rewrite SRT. ss. rewrite bind_trigger.
          eapply (step_choose _ cont (exist (fun 'ev => exists st1, step st0 (Some ev) st1) (event_sys fn args rv) H)). }
        esplits; et.
        ...
        { ss. rewrite bind_trigger. subst cont. ss.
          apply (@step_syscall fn args rv (fun rv0 : Any.t => exists st1, step st0 (Some (event_sys fn args rv0)) st1) (fun x : Any.t => Vis (Choose {st1 | step st0 (Some (event_sys fn args x)) st1}) (fun st1 : {st1 | step st0 (Some (event_sys fn args x)) st1} => decompile_STS step state_sort (st1 $)))).
  ...
  Theorem beh_preserved :
    exists itr, forall tr,
        ...
        <->
        of_state (compile_itree itr) itr tr.
  Proof.
    exists STS_itree. i.
    assert (A: of_state (compile_itree STS_itree) STS_itree tr
               =
