...
Module MemIA. Section MemIA.
  ...
  Proof using.
    ...
    steps_l. iDestruct "ASM" as "[% %]"; des; subst. rename _q into v.

    do 2 (hss_r; steps_r).
    des_ifs; cycle 1.
    { exfalso. simpl_bool; des.
    ...
    force_l ((Vptr (blk, 0%Z))↑); steps_l. force_l. steps_l.
    iMod (mem_ra_alloc with "B") as "[BLK WHT]"; eauto.
    iPoseProof (points_to_transform with "WHT") as "PT".
    force_l. iFrame. iSplit; et. fold blk.
  ...
  Lemma simF_free : ISim.sim_fun open MemA MemI (MemA.init_cond csl genv) IstFull (Some MemHdr.free).
  Proof using.
    init_simF.
    iDestruct "IST" as (? ? ? ?) "(% & [% [% [% [% >B]]]] & %)". des; subst; hss.

    destruct (or_else (pargs [Tptr] (or_else (arg↓) [])) (0, 0%Z)) as [b ofs] eqn: EQ.
    steps_l. force_l ((Vint 0)↑); steps_l.
    iDestruct "ASM" as "[[% PT] %]"; des; subst. hss. rename _q2 into v.

    steps_r. hss. steps_r.
    iPoseProof (mem_ra_lookup with "[B PT]") as "%HIT"; eauto; iFrame. des.
    rewrite HIT0.
    iMod (mem_ra_free with "[B PT]") as "B"; eauto; iFrame.

    steps_r. steps_l. forces_l. iSplit; et.
  ...
  Proof using.
    ...
    destruct (or_else (pargs [Tptr] (or_else (arg↓) []))(0,0%Z)) as [b ofs] eqn: EQ.
    steps_l. force_l ((mem_get mem_src b ofs)↑); steps_l.
    iDestruct "ASM" as "[[% PT] %]"; des; subst. hss.
    rename _q2 into v, _q4 into q.

    iPoseProof (mem_ra_lookup with "[B PT]") as "[% %]"; eauto; iFrame.
    forces_l. iFrame. erewrite mem_get_sound; et. iSplit; et.
  ...
  Proof using.
    ...
    steps_l. force_l ((Vint 0)↑); steps_l.
    iDestruct "ASM" as "[[% PT] %]"; des; subst. hss. rename _q4 into v.
    ...
    steps_l. steps_r. steps_l. steps_r. hss_r. steps_r. rewrite H2. steps_r.
    step. repeat (iSplit; eauto).
    iExists st_srcL, [_], _, _. repeat (iSplit; eauto).
  ...
  Proof using.
    ...
    iDestruct "ASM" as "[[% [P1 P2]] %]"; des; subst. hss.
    rename _q2 into succ, _q4 into v0, _q6 into q0, _q10 into v1, _q12 into q1.

    iPoseProof (mem_ra_cmp with "[B P1 P2]") as "%"; et; [iFrame|].

    forces_l. iFrame. rewrite H3. iSplit; et.

    steps_l. steps_r. hss_r; steps_r. destruct (Mem.vcmp mem_tgt p1 p2) as [r|] eqn: E; ss. steps_r.
    step. iSplit.
    { iPureIntro. inv H0. des_ifs; des_sumbool; subst; ss.
      rewrite /MemSpec.compare_val in H3; des_ifs; ss; des_ifs; ss.
    }
  ...
  Proof using.
    ...
    iDestruct "ASM" as "[[% [P [V1 V2]]] %]"; des; subst. hss.
    rename _q2 into succ, _q6 into v1, _q8 into q1, _q12 into v0, _q14 into q0, _q16 into v_cur'.
    ...
    { subst v_cur v_upd is_succ. erewrite mem_get_sound; et.
      repeat (iSplit; et).
      rewrite H3. destruct (dec succ 1) eqn: E; subst; et.
      destruct (dec (Vint succ) (Vint 1)) eqn: E'; et.
      exfalso. clear E'. inv e.
    }

    steps_r. inline_r. steps_r; hss_r; steps_r.
    hss_r; steps_r. rewrite H2. steps_r; hss_r; steps_r.
    inline_r; steps_r; hss_r; steps_r.
    hss_r; steps_r. rewrite H7. steps_r; hss_r; steps_r.
    add_ret_l (). iApply wsim_bind.
    instantiate (1:= λ '(st_s,_) '(st_t,_), ⌜st_s = _ ∧
      st_t = (_, (or_else (Mem.store mem_tgt (b,ofs) v_upd) mem_tgt)↑) :: _⌝%I).
    subst v_cur v_upd is_succ. erewrite mem_get_sound; et.
    iSplitL "".
    { destruct (dec succ 1) eqn: E; subst; s; et.
      - steps_r. inline_r. steps_r. hss. steps_r. hss. steps_r.
        rewrite H2. steps_r. hss. steps_r.
        step. iPureIntro; esplits; et. repeat f_equal.
        rewrite H3. s. et.
      - step. iPureIntro; esplits; et. repeat f_equal.
        rewrite H2. rewrite H3. s.
        destruct (dec (Vint succ) (Vint 1)) eqn: E'; s; et.
        { exfalso. clear E'. inv e. }
        destruct mem_tgt. ss. f_equal.
        extensionalities. des_ifs. simpl_bool. des.
        destruct (dec b _); ss; subst. destruct (dec ofs _); ss; subst. et.
    }

    iIntros (? ? ? ?) "%". des; subst.
    steps_l. steps_r. step.
    iSplit; et. rewrite H2; s.
