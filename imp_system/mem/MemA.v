...
Section syn_mem.
  ...
  Definition syn_mem_points_to {n} : (mblock * Z) → Qp → list val → GTerm.t n :=
...
Notation "loc ↦{ q } v" := (mem_points_to_singleton loc q v)%I : bi_scope.
Notation "loc ↦ v" := (mem_points_to_singleton loc 1%Qp v)%I : bi_scope.
Notation "loc |->{ q } vs" := (mem_points_to loc q vs)%I : bi_scope.
Notation "loc |-> vs" := (mem_points_to loc 1%Qp vs)%I : bi_scope.

Notation "loc ↦{ q } v" := (syn_mem_points_to_singleton loc q v)%SAT : SAT_scope.
Notation "loc ↦ v" := (syn_mem_points_to_singleton loc 1%Qp v)%SAT : SAT_scope.
Notation "loc |->{ q } vs" := (syn_mem_points_to loc q vs)%SAT : SAT_scope.
Notation "loc |-> vs" := (syn_mem_points_to loc 1%Qp vs)%SAT : SAT_scope.
