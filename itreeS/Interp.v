...
(** A plain event morphism [E ~> F] defines an itree morphism
    [itree E ~> itree F]. *)
Definition translateF {E: iEvent} {F: iEvent} {R: Type} (h : E ~> F) (rec: itree E R -> itree F R) (t : itreeF E R _) : itree F R  :=
  match t with
  | RetF x => Ret x
  | TauF t => Tau (rec t)
...
Definition translate {E: iEvent} {F: iEvent} (h : E ~> F)
  : itree E ~> itree F
...
Definition interp {E: iEvent} {M: Type -> Type}
           {FM : Functor M} {MM : Monad M} {IM : MonadIter M}
           (h : E ~> M) :
  itree E ~> M
  :=
  fun (R: Type) =>
  iter (fun (t: itree E R) =>
