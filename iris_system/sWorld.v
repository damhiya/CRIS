...
Section syn_inv.
  ...
  Context `{!invGS Σ Γ, !SRFIntp.inG _ α inv_interp β}.
...
Module Adequacy.
  ...
  Instance β {Σ : GRA} {Γ : HRA} `{!subG Γ Σ, !invGS Σ Γ} : SRFIntp.t :=
    λ n,
      match n with
      | 0 => SL.t
      | _ => inv_interp
  ...
  Instance intpG {Σ : GRA} {Γ : HRA} `{!subG Γ Σ, !invGS Σ Γ} :
    SRFIntp.inG (@SL.syntax τ Γ) α (@SL.t τ α Γ Σ _) β.
  Proof. econs; instantiate (1:=0); ss. Qed.

  Instance invintpG {Σ : GRA} {Γ : HRA} `{!subG Γ Σ, !invGS Σ Γ} :
    SRFIntp.inG inv_syntax α inv_interp β.
  Proof. econs; instantiate (1:=1); ss. Qed.

  Instance sinvgs {Σ : GRA} {Γ : HRA} `{!subG Γ Σ, !invGS Σ Γ} : sinvGS Σ Γ α β τ.
