...
Section read.
  ...
  Lemma simF_read : ISim.sim_fun open MA MI (init_cond syn size) PFMemIA.Ist (Some PFMemHdr.read).
  Proof.
    init_simF.
    step_l. destruct _q as [[|i] X]; steps_l; rename _q into arg'.
    { (* non-atomic read *)
      destruct X as [[[[[tid loc] ord] val] q] 𝓥].
      ...
      { iFrame. iSplit; eauto. iExists val'.
        iSplit; eauto.
        rewrite own_loc_na_eq /own_loc_na_def /view_at.
        iExists from, ts, LT, released, na.
        iSplit.
        { iSplit; eauto. 
          iPureIntro; rewrite /alloc_local; split.
          { ss. do 2 eapply AllocView.join_l; eauto. }
          { eexists ts, (from, Message.message val released na).
    ...
    { destruct i; last (destruct i; inv X).
      destruct X as [[[[[[[[[[tid loc] ord] ζ] ζ'] t] γ] q] mode] 𝓥] Vb].
      iDestruct "ASM" as "[[[-> %ORDRLX] [SEEN [PT TV]]] ->]". hss.
      iDestruct "IST" as "[%gl [%ths [%Vcut [[-> [%CUT [%CUTCL [%WF [%WF2 [%PFG %PFL]]]]]] [HA [TA FA]]]]]]".
      steps_r. hss. steps_r.
      rewrite /PFMemI.check_ident.
      des_ifs; last (iPoseProof (tview_both_valid with "TA TV") as "%F"; des; ss; clarify).
