...
Lemma gsim_adequacy_spin_aux
  (r : _ -> Prop)
  (CIH : ∀ ps pt itr_src itr_tgt,
      gsim eq ps pt itr_src itr_tgt → Beh.state_spin itr_tgt → r itr_src)
  ps'
  (PRE: ∀ ps pt itr_src itr_tgt
           (LT: smj_ltb ps ps')
           (SIM : gsim eq ps pt itr_src itr_tgt)
...
Proof.
  ...
  eapply gsim_ind, SIM. clear - CIH PRE.
...
Proof.
  ...
  do 3 (eapply gsim_adequacy_spin_aux; try (by left; refl); eauto; i).
  exfalso. eauto using smj_ltb_not_three.
Qed.

Local Ltac auto_gsim SIM0 SIM1 x x0 x1 :=
  try itree_clarify x;
...
Lemma gsim_adequacy_ret_aux
  pt' retv
  (PRE: ∀ ps pt itr_src
           (LE: smj_ltb pt pt')
           (SIM: gsim eq ps pt itr_src (Ret retv)),
...
Proof.
  ...
  eapply gsim_ind, SIM. clear - PRE.
  intros ps pt itr_src itr_tgt SIM LE EQ. subst.
  depdes SIM; auto_gsim SIM0 SIM1 x x0 x1.
  eapply PRE; eauto.
  destruct LE; subst; eauto using smj_ltb_trans.
Qed.

Lemma gsim_adequacy_ret
  retv ps pt itr_src
  (SIM: gsim eq ps pt itr_src (Ret retv))
  :
  Beh.of_itree itr_src (Tr.done retv).
Proof.
  do 3 (eapply gsim_adequacy_ret_aux; try (by left; refl); eauto; i).
  exfalso. eauto using smj_ltb_not_three.
Qed.

Lemma gsim_adequacy_tau_aux
  pt' r itr_tgt tr
  (STEP: ∀ ps pt itr_src
            (SIM: gsim eq ps pt itr_src itr_tgt),
  ...
  (PRE: ∀ ps pt itr_src              
           (LE: smj_ltb pt pt')
           (SIM: gsim eq ps pt itr_src (tau;; itr_tgt)),
      paco2 Beh._of_itree r itr_src tr)
  ps pt itr_src
  (LE: smj_le pt pt')
  (SIM: gsim eq ps pt itr_src (tau;; itr_tgt))
...
Proof.
  ...
  revert LE EQ. pattern ps, pt, itr_src, itr_tgt'.
  eapply gsim_ind, SIM. clear - STEP PRE.
  intros ps pt itr_src itr_tgt' SIM LE EQ. subst.
  depdes SIM; auto_gsim SIM0 SIM1 x x0 x1.
  eapply PRE; eauto.
  destruct LE; subst; eauto using smj_ltb_trans.
...
Lemma gsim_adequacy_tau
  ...
  paco2 Beh._of_itree r itr_src tr.
Proof.
  do 3 (eapply gsim_adequacy_tau_aux; try (by left; refl); eauto; i).
  exfalso. eauto using smj_ltb_not_three.
Qed.

Lemma gsim_adequacy_hang_aux
  pt' r I O fn args k
  (CIH: ∀ ps pt itr_src itr_tgt
           (SIM: gsim eq ps pt itr_src itr_tgt),
...
Lemma gsim_adequacy_interact_aux
...
Proof.
  remember (r <- trigger (@IO I O fn args);; k r) as itr_tgt eqn: EQ in SIM.
  revert LE EQ. pattern ps, pt, itr_src, itr_tgt.
  eapply gsim_ind, SIM. clear - CIH BEH PRE.
  intros ps pt itr_src itr_tgt SIM LE EQ. subst.
  depdes SIM; auto_gsim SIM0 SIM1 x x0 x1.
  eapply PRE; eauto.
  destruct LE; subst; eauto using smj_ltb_trans.
...
Lemma gsim_adequacy_interact
  ...
  paco2 Beh._of_itree r itr_src (Tr.interact (obs_io fn args retv) tr).
Proof.
  do 3 (eapply gsim_adequacy_interact_aux; try (by left; refl); eauto; i).
  exfalso. eauto using smj_ltb_not_three.
Qed.

Lemma gsim_adequacy_choose_aux
  pt' r X retv k tr
  (STEP: ∀ ps pt itr_src
            (SIM: gsim eq ps pt itr_src (k retv)),
      paco2 Beh._of_itree r itr_src tr)
  (BEH: Beh.of_itree (k retv) tr)
  (PRE: ∀ ps pt itr_src
           (LE: smj_ltb pt pt')
           (SIM: gsim eq ps pt itr_src (r <- trigger (Choose X);; k r)),
      paco2 Beh._of_itree r itr_src tr)
  ps pt itr_src
  (LE: smj_le pt pt')
  (SIM: gsim eq ps pt itr_src (r <- trigger (Choose X);; k r))
...
Proof.
  ...
  revert LE EQ. pattern ps, pt, itr_src, itr_tgt.
  eapply gsim_ind, SIM. clear - STEP BEH PRE.
  intros ps pt itr_src itr_tgt SIM LE EQ. subst.
  depdes SIM; auto_gsim SIM0 SIM1 x x0 x1.
  eapply PRE; eauto.
  destruct LE; subst; eauto using smj_ltb_trans.
...
Lemma gsim_adequacy_choose
  ...
  paco2 Beh._of_itree r itr_src tr.
Proof.
  do 3 (eapply gsim_adequacy_choose_aux; try (by left; refl); eauto; i).
  exfalso. eauto using smj_ltb_not_three.
Qed.

Lemma gsim_adequacy_take_aux
  pt' r X k tr
  ...
  (PRE: ∀ ps pt itr_src
           (LE: smj_ltb pt pt')
           (SIM: gsim eq ps pt itr_src (r <- trigger (Take X);; k r)),
      paco2 Beh._of_itree r itr_src tr)
  ps pt itr_src
  (LE: smj_le pt pt')
  (SIM: gsim eq ps pt itr_src (r <- trigger (Take X);; k r))
...
Lemma gsim_adequacy_take
  ...
  paco2 Beh._of_itree r itr_src tr.
Proof.
  do 3 (eapply gsim_adequacy_take_aux; try (by left; refl); eauto; i).
  exfalso. eauto using smj_ltb_not_three.
Qed.
...
Proof.
  ...
  depdes PR; des; hdes.
  - eapply paco2_mon; try eapply gsim_adequacy_ret; eauto; ss.
  - pstep. econs. econs.
  - eapply paco2_mon; try eapply gsim_adequacy_spin; eauto; ss.
  - eapply paco2_mon; try eapply gsim_adequacy_tau; eauto; ss.
  - eapply paco2_mon; try eapply gsim_adequacy_hang; eauto; ss.
  - eapply paco2_mon; try eapply gsim_adequacy_interact; eauto; ss.
  - eapply paco2_mon; try eapply gsim_adequacy_choose; eauto; ss.
  - eapply paco2_mon; try eapply gsim_adequacy_take; eauto; ss.
Qed.
