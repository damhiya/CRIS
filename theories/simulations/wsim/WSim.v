...
Section wsim.
  ...
  Lemma wsim_call_sandbox fn arg k_s k_t (mask_src mask_tgt : _ → bool) scopes_src scopes_tgt :
    ...
    (∀ ret nths' st_s' st_t'
      ...
      sim Ep r g RR true true nths' (st_s', k_s ret) (st_t', k_t ret)) ⊢
    sim Ep r g RR ps pt nths
      (st_s, HModTr.sandbox mask_src scopes_src (trigger (Call fn arg)) >>= k_s)
      (st_t, HModTr.sandbox mask_tgt scopes_tgt (trigger (Call fn arg)) >>= k_t).
  Proof using.
    iIntros (??) "SIM"; rewrite !SBRed.call; des_ifs; ss; iApply wsim_call; done.
  Qed.
  ...
  Lemma wsim_inline_src_sandbox fn arg f_s k_s i_t (mask : _ → bool) scopes :
    alist_find fn fl_s = Some f_s →
    mask fn →
    sim Ep r g RR true pt nths
      (st_s, x <- (ret <- (f_s arg);; (tau;; Ret ret));; (k_s x))
      (st_t, i_t) ⊢
    sim Ep r g RR ps pt nths
      (st_s, HModTr.sandbox mask scopes (trigger (Call fn arg)) >>= k_s)
      (st_t, i_t).
  Proof using.
    i. iIntros "ISIM". rewrite SBRed.call. des_ifs; ss. iApply wsim_inline_src; eauto.
  Qed.

  Lemma wsim_inline_tgt fn arg f_t i_s k_t :
  ...
  Lemma wsim_inline_tgt_sandbox fn arg f_t i_s k_t (mask : _ → bool) scopes :
    alist_find fn fl_t = Some f_t →
    mask fn →
    sim Ep r g RR ps true nths
      (st_s, i_s)
    ...
    sim Ep r g RR ps pt nths
      (st_s, i_s)
      (st_t, HModTr.sandbox mask scopes (trigger (Call fn arg)) >>= k_t).
  Proof using.
  ...
  Lemma wsim_spawn_sandbox fn args k_s k_t (mask_src mask_tgt : _ → bool) scopes_src scopes_tgt :
    mask_src fn → mask_tgt fn →
    sim Ep r g RR true true (S nths) (st_s, k_s nths) (st_t, k_t nths) ⊢
    sim Ep r g RR ps pt nths
  ...
  Proof using.
    i. iIntros "ISIM". rewrite !SBRed.spawn. des_ifs; ss. iApply wsim_spawn. iFrame.
  Qed.
  ...
  Lemma wsim_unwrapN_src X (x : option X) k_s i_t :
    (∃ x', ⌜x = Some x'⌝ ∗ sim Ep r g RR ps pt nths (st_s, k_s x') (st_t, i_t)) ⊢
    sim Ep r g RR ps pt nths (st_s, unwrapN x >>= k_s) (st_t, i_t).
  Proof using. iIntros "H". iDestruct "H" as (x') "[% H]". subst. hred_l. iApply "H". Qed.

  Lemma wsim_sput_src_sandbox mask scopes k v k_s i_t :
    In k.1 scopes →
    sim Ep r g RR true pt nths
  ...
  Lemma wsim_sget_src_sandbox mask scopes k k_s i_t :
    In k.1 scopes →
    sim Ep r g RR true pt nths
  ...
  Lemma wsim_sput_tgt_sandbox mask scopes k v i_s k_t :
    In k.1 scopes →
    sim Ep r g RR ps true nths
  ...
  Lemma wsim_sget_tgt_sandbox mask scopes k i_s k_t :
    In k.1 scopes →
    sim Ep r g RR ps true nths
  ...
  Proof using.
    unseal; iIntros "SIM"; ss; iPoseProof ("SIM" with "[]") as "SIM"; done.
  Qed.
