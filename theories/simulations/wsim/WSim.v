...
Section FSPEC.
  ...
  Definition wsim_fspec (υ : univ_id) (fsp : fspec) : fspec :=
    mk_fspec (meta := meta fsp)
      (λ x varg arg, wsim_ginv υ ⊤ ∗ precond fsp x varg arg)%I
      (λ x vret ret, wsim_ginv υ ⊤ ∗ postcond fsp x vret ret)%I.
...
Class WP `{!sinvG Γ Σ α β τ _I _G} 
...
Section wsim.
  ...
  Proof using. unseal; iIntros "RR I". iApply isim_ret. iFrame. Qed.
  ...
  Lemma wsim_assume_src P r g k_s i_t E :
  ...
  Lemma wsim_assume_precise_src P r g k_s i_t E
    :
    precise P
    ∗
    (P -∗ wsim t υ ν E r g R_s R_t RR true pt nths
      (st_s, k_s tt) (st_t, i_t)) ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, trigger (AssumePrecise P) >>= k_s) (st_t, i_t).
  Proof using.
    unseal. iIntros "[P H] I". iApply isim_assume_precise_src; eauto.
    iFrame. iIntros "P". iApply ("H" with "P I").
  Qed.

  Lemma wsim_assume_tgt P r g i_s k_t E :
    P ∗ wsim t υ ν E r g R_s R_t RR ps true nths
      (st_s, i_s) (st_t, k_t tt) ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, i_s) (st_t, trigger (Assume P) >>= k_t).
  Proof using. unseal; iIntros "[P RR] I". iApply isim_assume_tgt; eauto. iFrame. iApply "RR". ss. Qed.

  Lemma wsim_assume_precise_tgt P r g i_s k_t E :
    (precise P -∗ P ∗ wsim t υ ν E r g R_s R_t RR ps true nths
          (st_s, i_s) (st_t, k_t tt)) ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
  ...
  Lemma wsim_assume_precise_both P r g k_s k_t E :
    (wsim t υ ν E r g R_s R_t RR true true nths
      (st_s, k_s tt) (st_t, k_t tt))
    ⊢ wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, trigger (AssumePrecise P) >>= k_s) (st_t, trigger (AssumePrecise P) >>= k_t).
  Proof using.
    unseal. iIntros "RR I". iApply isim_assume_precise_both; eauto.
    iApply "RR". et.
  Qed.
  ...
  Lemma wsim_spawn_sandbox r g fn args k_s k_t E img_src img_tgt (msk_src msk_tgt:_→bool) scp_src scp_tgt:
    (msk_src fn → msk_tgt fn) →
    wsim t υ ν E r g R_s R_t RR true true (S nths)
      (st_s, k_s nths) (st_t, k_t nths) ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
...
Section Proph.
  ...
  Lemma wsim_assume_proph_src {X R} Pre (Post: _→R→_) r g k_s i_t E
    :
    (∃ P Q,
      (precise P) ∗
      (∀ x:X, Pre x ==∗ P ∗ (∀ ret, Q ret ==∗ Post x ret)) ∗
      (P -∗ wsim t υ ν E r g R_s R_t RR true pt nths (st_s, k_s Q) (st_t, i_t)))
    ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, (AssumeProph Pre Post) >>= k_s) (st_t, i_t).
  Proof.
  ...
  Lemma wsim_assume_proph_src_advanced {X R} (Pre: X→_) Post r g k_s i_t E
    :
    (∃ I P Q,
      I ∗ (precise P) ∗
      (∀ x, ∃ T, (I ∗ Pre x -∗ □ T) ∗ ((□ T) ∗ Pre x ==∗ P ∗ (∀ ret: R, Q ret ==∗ Post x ret))) ∗
      (I ∗ P -∗ wsim t υ ν E r g R_s R_t RR true pt nths (st_s, k_s Q) (st_t, i_t)))
    ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, (AssumeProph Pre Post) >>= k_s) (st_t, i_t).
  Proof.
  ...
  Lemma wsim_assume_proph_src_simple {X R} Pre (Post: _→R→_) r g k_s i_t E
    :
    (∃ x: X, precise (Pre x) ∗
      ∀ x', Pre x' -∗
        ⌜x' = x⌝ ∗
        wsim t υ ν E r g R_s R_t RR true pt nths (st_s, k_s (Post x)) (st_t, i_t))
    ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, (AssumeProph Pre Post) >>= k_s) (st_t, i_t).
  Proof.
  ...
  Lemma wsim_assume_proph_tgt {X R} Pre (Post: _→R→_) r g i_s k_t E
    :
    (∃ x: X, Pre x
       ∗
       ∀ Q, (∀ ret, Q ret ==∗ Post x ret) -∗
       wsim t υ ν E r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t Q))
    ⊢
    wsim t υ ν E r g R_s R_t RR ps pt nths
      (st_s, i_s) (st_t, (AssumeProph Pre Post) >>= k_t).
  Proof.
    rewrite wsim_eq /wsim_def.
    iIntros "[% [P H]] W".
