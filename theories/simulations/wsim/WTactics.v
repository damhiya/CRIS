...
Ltac wcall hyps :=
  (wnorm with do 1 iApply wsim_call_sandbox); [try prove_sb_cond|try prove_sb_cond|
  iSplitL hyps; [try done| iIntros "% % % % % %"; iIntrosFresh "IST"];
  move_aux].

Ltac wspawn :=
  (wnorm with do 1 iApply wsim_spawn_sandbox); [try prove_sb_cond|try prove_sb_cond|].
