...
Proof using.
  ...
  cut (∀ (f: emask * fbody) (WF: Mod.wf md) (SCP: ∀ X (e: crisE X), f.1 _ (subevent _ e) → (msk_scp (Mod.scopes md)) _ (subevent _ e)),
  isim_fsem
  ...
  { econs; ss; try refl; eauto; i.
    { r. rewrite !lookup_fmap.
      ...
      { destruct (decide (k.1 ∈ Mod.scopes md)); eauto. }
      { destruct (decide (k.1 ∈ Mod.scopes md)); eauto. }
    }
  }
  ...
  eapply isim_coind. intros ? _ CIH [msk [SCP [ps [pt [st it]]]]]; s.
  ...
  - rewrite SBRed.bind SBRed.vis !vis_trigger. des_ifs; cycle 1.
    ...
    iforce_l. iFrame. isteps_l. rewrite !SBRed.ret bind_ret_l.
