...
Lemma cancel_pre `{Σ: GRA} md sp:
  ∀ (rs0: Σ) r_s r_t srcs tgts cid st ps pt varg X X' Po Po' itrS ktrT k
    ...
    (RS : Own r_s ⊢ |==> Own r_t)
    (LEN : cid < length srcs)
    (x0 : srcs !! cid = Some (HModTr.trans (tau;; itrS)))
      ...
      ∧ ∃ P' : X' → Any.t → Any.t → iProp Σ,
          ...
          ∧ ∀ x : X,
              ∃ x' : X',
                ...
                ∧ upaco3 (elim_rel_def sp) bot3 Any.t itrS (ktrT (x, x', varg))),
...
Proof.
  ...
  destruct KTR; subst; des_safe.
  {
    ...
    }
    {
      ...
      des. subst. ired. pclearbot. eapply KEY; et.
