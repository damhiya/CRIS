...
Definition HoareSpawnE (fspo: option fspec) (sspo: bool) fn varg : itree crisE nat :=
  ...
  | Some fsp, true =>
      ...
      trigger (Guarantee (YIELD tid -∗ TID tid -∗ winv (⊤, ⊤) -∗ precond fsp x varg arg));;; tau;;
      Ret tid
  | None, true =>
...
Definition elim_spawnee_precond (X : Type) P (arg : Any.t) : itree crisE (X * Any.t) :=
  x <- trigger (Take X);; tau;;
  varg <- trigger (Take Any.t);; tau;;
  (* trigger (Assume (TID stid ∗ YIELD stid ∗ winv (↑N, ↑N)));;; tau;; *)
  trigger (Assume (P x varg arg));;; tau;;
...
Definition elim_spawnee_postcond {X : Type} Q (x : X) (vret : Any.t) : itree crisE Any.t :=
  ret <- trigger (Choose Any.t);; tau;;
  (* trigger (Guarantee (TID stid ∗ YIELD stid ∗ winv (↑N, ↑N)));;; tau;; *)
...
Lemma MIRed_HoareFun
  ...
  '(x, varg) : _ <- @elim_spawnee_precond _ pre arg;;
  vret <-
    inline_body
      (sandboxed_prog (SMod.to_mod sp md))
      (SB.sandbox msk (SModTr.trans sp (bd varg)));;
  elim_spawnee_postcond post x vret.
Proof using.
...
Lemma MIRed_HoareCall md sp fn varg
  (msk msk0: emask) fspo fspo0 bd0 m m0 pre pre0 post post0
  ...
  :
  inline_body (sandboxed_prog (SMod.to_mod sp md)) (SB.sandbox msk (SModTr.HoareCall fspo fn varg))
  =
  (* head *)
  '(x, x0,arg):_ <- elim_precond pre pre0 varg ;;
  (* body *)
  vret0 <- inline_body (sandboxed_prog (SMod.to_mod sp md)) 
                       (SB.sandbox msk0 (SModTr.trans sp (bd0 arg)));;
  (* tail *)
...
Proof using.
  ...
  extensionalities. ired. destruct H1. ired. do 2 f_equal.
  rewrite !SBRed.ret bind_ret_l.
  rewrite !SBRed.bind !SBRed.vis /= IM vis_trigger !bind_bind MIRed.core. do 2 f_equal.
  extensionalities. ired. do 2 f_equal.
  rewrite !SBRed.ret bind_ret_l.
...
Lemma elim_rel_cancel (md: SMod.t) T msk (itr: itree _ T)
  ...
  @elim_rel (SMod.conc_sp_from md) T ε
    ...
    (inline_body (sandboxed_prog (SMod.to_mod (SMod.conc_sp_from md) md)) 
      (SB.sandbox msk (SModTr.trans (SMod.conc_sp_from md) itr))).
Proof using.
