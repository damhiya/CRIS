...
Section CANCEL.
  ...
  Lemma cancel_aux_head
    ...
    (KTR: ∀ tid (m: X0) varg, upaco3 (@elim_rel_def _ md ginv _) bot3 ((tid, existT X0 m) :: l) (ktrS varg) (ktrT (tid, m, tid, m, varg)))
    (SRC : srcs !! cid = Some (Ret ();;; interp_hp (tau;; ktrS varg)))
    (TGT : tgts !! cid = Some (Ret ();;; interp_hp (cancel_term md ginv cid meta Q (x <- hmod_elim_head X0 P varg;; ktrT x))))
