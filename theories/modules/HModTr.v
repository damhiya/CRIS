...
Module SBRed. Section SBRed.
  ...
  Proof using. unfold HModTr.sandbox. rewrite (bisim_is_eq (translate_bind _ _ _)); eauto. Qed.

  Lemma tau A scopes (itr : itree hmodE A) :
    HModTr.sandbox scopes (tau;; itr) = tau;; (HModTr.sandbox scopes itr).
  Proof using. unfold HModTr.sandbox. rewrite (bisim_is_eq (translate_tau _ _)); eauto. Qed.

  Lemma ret A (a : A) scopes :
    HModTr.sandbox scopes (Ret a) = Ret a.
  Proof using. unfold HModTr.sandbox. rewrite (bisim_is_eq (translate_ret _ _)); eauto. Qed.
  ...
  Lemma vis_put {R} scopes k v (ktr : () -> itree hmodE R) :
    ...
    = if existsb (String.eqb k.1) scopes
      ...
      else vis (Choose ()) (fun x => HModTr.sandbox scopes (ktr x)).
  ...
  Lemma vis_get {R} k scopes (ktr : Any.t -> itree hmodE R) :
    ...
    = if existsb (String.eqb k.1) scopes
      ...
      else vis (Choose Any.t) (fun x => HModTr.sandbox scopes (ktr x)).
  Proof using. destruct k; ss. eapply observe_eta; ss. des_ifs. Qed.
  ...
  Proof using.
    unfold HModTr.sandbox, trigger. destruct k. s.
    rewrite (bisim_is_eq (translate_vis _ _ _ _)). ss.
