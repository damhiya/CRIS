...
Class mono_listG (A : Type) (Σ : GRA) :=
  MonoListG { #[local] mono_list_inG :: inG (mono_listR (leibnizO A)) Σ }.

Definition mono_listΣ (A : Type) : GRA :=
  #[ (mono_listR (leibnizO A))].

Global Instance subG_mono_listΣ {A Σ} :
  subG (mono_listΣ A) Σ → (mono_listG A) Σ.
Proof. solve_inG. Qed.

Global Opaque mono_listΣ.
...
Section mono_list_own.
  ...
  Lemma mono_list_own_alloc l :
    ⊢ o=> ∃ γ, mono_list_auth_own γ 1 l ∗ mono_list_lb_own γ l.
  Proof.
...
Global Instance subHG_mono_list {A Γ Σ}:
  subHG Γ Σ → (mono_listG A) Γ → (mono_listG A) Σ.
Proof. intros ? []. constructor. apply _. Defined.

(* Ideally automatic, but unification of HRA and gFunctors is problematic? *)
Local Instance inGΓ_mono_list {A} {Γ : HRA} :
  (mono_listG A) Γ → inG (mono_listR (leibnizO A)) Γ.
...
Section syn_mono_list_own.
  ...
  *)
  Context `{ML: !mono_listG A Γ}.
  Implicit Types (γ : gname) (l : list A) (i : nat) (a : A).

  Definition syn_mono_list_auth_own {n} γ (q : Qp) l : GTerm.t n :=
    sown γ (●ML{#q} (l : listO (leibnizO A))).

  Definition syn_mono_list_lb_own {n} γ l : GTerm.t n :=
    sown γ (◯ML (l : listO (leibnizO A))).

  Definition syn_mono_list_idx_own {n} γ i a : GTerm.t n :=
    ∃ l : τ{list A}%SAT, ⌜ l !! i = Some a ⌝ ∗ syn_mono_list_lb_own γ l.
