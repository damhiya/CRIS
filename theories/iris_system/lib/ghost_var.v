...
(** The CMRA we need. *)
Class ghost_varG (Σ : GRA) (A : Type) := GhostVarG {
  #[local] ghost_var_inG :: inG (dfrac_agreeR $ leibnizO A) Σ;
}.
Global Hint Mode ghost_varG ! - : typeclass_instances.

Definition ghost_varΣ (A : Type) : GRA :=
  #[ (dfrac_agreeR $ leibnizO A) ].
...
Global Instance subHG_ghost_var {A Γ Σ}:
  subHG Γ Σ → ghost_varG Γ A → ghost_varG Σ A.
Proof. intros ? []. constructor. apply _. Defined.

(* FIXME: Ideally automatic, but unification of HRA and GRA is problematic? *)
Local Instance inGΓ_ghost_var {A} {Γ : HRA} :
  ghost_varG Γ A → inG (dfrac_agreeR $ leibnizO A) Γ.
...
Section syn_ghost_var.
  ...
  Definition syn_ghost_var {n} γ (q : Qp) a : GTerm.t n :=
    sown γ (to_frac_agree (A:=leibnizO A) q a).
