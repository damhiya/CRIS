...
Class ghost_mapG (Σ : GRA) (K V : Type) `{Countable K} := GhostMapG {
  #[local] ghost_map_inG :: inG (gmap_viewR K (agreeR (leibnizO V))) Σ;
}.
Global Arguments ghost_mapG _ _ _ {_} {_}.

Definition ghost_mapΣ (K V : Type) `{Countable K} : GRA :=
  #[ (gmap_viewR K (agreeR (leibnizO V))) ].
...
Section lemmas.
  ...
  Lemma ghost_map_alloc m :
    ⊢ o=> ∃ γ, ghost_map_auth γ 1 m ∗ [∗ map] k ↦ v ∈ m, k ↪[γ] v.
  Proof.
    unseal. intros.
    iMod (own_alloc
      (gmap_view_auth (V:=agreeR (leibnizO V)) (DfracOwn 1) ∅))
      as (γ) "Hauth".
    { apply gmap_view_auth_valid. }
    iExists γ.
    rewrite -big_opM_own_1 -own_op. iMod (own_update with "Hauth") as "$"; last done.
    etrans; first apply (gmap_view_alloc_big _ (to_agree <$> m) (DfracOwn 1)).
    - apply map_disjoint_empty_r.
    - done.
    - by apply map_Forall_fmap.
  ...
  Lemma ghost_map_alloc_empty :
    ⊢ o=> ∃ γ, ghost_map_auth γ 1 (∅ : gmap K V).
  Proof.
  ...
  (* Lemma ghost_map_update_big {γ m} m0 m1 :
    dom m0 = dom m1 →
    ghost_map_auth γ 1 m -∗
    ([∗ map] k↦v ∈ m0, k ↪[γ] v) ==∗
...
Global Instance subHG_ghost_map `{Countable K} {V Γ Σ}:
  subHG Γ Σ → ghost_mapG Γ K V → ghost_mapG Σ K V.
Proof. intros ? []. constructor. apply _. Defined.

(* FIXME: Ideally automatic, but unification of HRA and gFunctors is problematic? *)
Local Instance inGΓ_ghost_map `{Countable K} {V} {Γ : HRA} :
  ghost_mapG Γ K V → inG (gmap_viewR K (agreeR (leibnizO V))) Γ .
...
Section syn_ghost_map.
  ...
  Definition syn_ghost_map_auth {n} γ q m : GTerm.t n :=
    sown γ (gmap_view_auth (V:=agreeR $ leibnizO V) (DfracOwn q) (to_agree <$> m)).

  Definition syn_ghost_map_elem {n} γ k dq v : GTerm.t n :=
    sown γ (gmap_view_frag (V:=agreeR $ leibnizO V) k dq (to_agree v)).
