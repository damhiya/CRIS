...
Definition to_sp_context `{Σ : GRA} (spu : list string) (spk : alist string fspec) :=
  to_sp (List.map (fun fn => (fn, fspec_trivial)) spu ++ spk).

Definition to_closed_sp `{Σ : GRA} (l : alist string fspec) : string -> option fspec :=
...
Section HEADER.
  ...
  Variant fn_has_spec (sp : string -> option fspec) (fn : string) (fsp : fspec) : Prop :=
  | fn_has_spec_intro fsp1
      (FIND : sp fn = Some fsp1)
      (WEAK : fspec_weaker fsp fsp1).
  Hint Constructors fn_has_spec : core.

  Lemma fn_has_spec_weaker (sp : string -> option fspec) (fn : string) (fsp0 fsp1 : fspec)
  ...
  Definition sp_sub (sp0 sp1 : string -> option fspec) : Prop :=
    ∀ fn fsp (FIND : sp0 fn = Some fsp), sp1 fn = Some fsp.

  Definition sp_incl (spl : alist string fspec) (gsp : string -> option fspec) : Prop :=
  ...
  Definition sp_weaker (sp0 sp1 : string -> option fspec) : Prop :=
