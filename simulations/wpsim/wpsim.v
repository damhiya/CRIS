...
Section wpsim.
  ...
  Lemma wpim_call r g R_s R_t RR ps pt nths st_s st_t fn arg k_s k_t E :
    Ist nths st_s st_t ∗
    (∀ ret nths' st_s' st_t'
      (NODS : List.NoDup (List.map fst st_s'))
      (NODD : List.NoDup (List.map fst st_t')),
      Ist nths' st_s' st_t' -∗
      wpsim r g R_s R_t RR true true nths' (st_s', k_s ret) (st_t', k_t ret) E) ⊢
    wpsim r g R_s R_t RR ps pt nths
  ...
  Lemma wpsim_io r g R_s R_t RR ps pt nths st_s st_t fn I O (arg : I) k_s k_t E :
    (∀ (ret : O), wpsim r g R_s R_t RR true true nths (st_s, k_s ret) (st_t, k_t ret) E) ⊢
    wpsim r g R_s R_t RR ps pt nths
  ...
  Lemma wpsim_tau_src r g R_s R_t RR ps pt nths st_s st_t i_s i_t E :
    wpsim r g R_s R_t RR true pt nths (st_s, i_s) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, tau;; i_s) (st_t, i_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_tau_src; iApply "RR"; iFrame. Qed.

  Lemma wpsim_tau_tgt r g R_s R_t RR ps pt nths st_s st_t i_s i_t E :
    wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, tau;; i_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_tau_tgt; iApply "RR"; iFrame. Qed.

  Lemma wpsim_inline_src r g R_s R_t RR ps pt nths st_s st_t fn arg f_s k_s i_t E :
    alist_find fn fl_s = Some f_s →
    wpsim r g R_s R_t RR true pt nths
      (st_s, x <- (ret <- (f_s arg);; (tau;; tau;; Ret ret));; (k_s x))
      (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (Call fn arg) >>= k_s) (st_t, i_t) E.
  Proof. i; unseal; iIntros "RR I". iApply isim_inline_src; eauto. iApply "RR"; iFrame. Qed.

  Lemma wpsim_inline_tgt r g R_s R_t RR ps pt nths st_s st_t fn arg i_s f_t k_t E :
    alist_find fn fl_t = Some f_t →
    wpsim r g R_s R_t RR ps true nths
      (st_s, i_s)
      (st_t, x <- (ret <- (f_t arg);; (tau;; tau;; Ret ret));; (k_t x)) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (Call fn arg) >>= k_t) E.
  Proof. i; unseal; iIntros "RR I". iApply isim_inline_tgt; eauto. iApply "RR"; iFrame. Qed.

  Lemma wpsim_take_src X r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    (∀ x, wpsim r g R_s R_t RR true pt nths (st_s, k_s x) (st_t, i_t) E) ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (Take X) >>= k_s) (st_t, i_t) E.
  Proof.
    unseal; iIntros "RR I". iApply isim_take_src; eauto. iIntros (x); iApply "RR"; iFrame.
  Qed.

  Lemma wpsim_take_tgt X r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    (∃ x, wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t x) E) ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (Take X) >>= k_t) E.
  Proof.
    unseal; iIntros "[%x RR] I". iApply isim_take_tgt; eauto. iExists _; iApply "RR"; iFrame.
  Qed.

  Lemma wpsim_choose_src X r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    (∃ x, wpsim r g R_s R_t RR true pt nths (st_s, k_s x) (st_t, i_t) E) ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (Choose X) >>= k_s) (st_t, i_t) E.
  Proof.
    unseal; iIntros "[%x RR] I". iApply isim_choose_src; eauto. iExists _; iApply "RR"; iFrame.
  Qed.

  Lemma wpsim_choose_tgt X r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    (∀ x, wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t x) E) ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (Choose X) >>= k_t) E.
  Proof.
    unseal; iIntros "RR I". iApply isim_choose_tgt; eauto. iIntros (x); iApply "RR"; iFrame.
  Qed.

  Lemma wpsim_sput_src k v r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    wpsim r g R_s R_t RR true pt nths (alist_upd k v st_s, k_s tt) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (SPut k v) >>= k_s) (st_t, i_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_sput_src; eauto. iApply "RR"; iFrame. Qed.

  Lemma wpsim_sput_tgt k v r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    wpsim r g R_s R_t RR ps true nths (st_s, i_s) (alist_upd k v st_t, k_t tt) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (SPut k v) >>= k_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_sput_tgt; eauto. iApply "RR"; iFrame. Qed.

  Lemma wpsim_sget_src k r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    wpsim r g R_s R_t RR true pt nths (st_s, k_s (or_else (alist_find k st_s) tt↑)) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (SGet k) >>= k_s) (st_t, i_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_sget_src; eauto. iApply "RR"; iFrame. Qed.

  Lemma wpsim_sget_tgt k r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t (or_else (alist_find k st_t) tt↑)) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (SGet k) >>= k_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_sget_tgt; eauto. iApply "RR"; iFrame. Qed.

  Lemma wpsim_assume_src (P : iProp Σ) r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    (P -∗ wpsim r g R_s R_t RR true pt nths (st_s, k_s tt) (st_t, i_t) E) ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (Assume P) >>= k_s) (st_t, i_t) E.
  Proof.
    unseal; iIntros "RR I". iApply isim_Assume_src; eauto. iIntros "P".
    iApply ("RR" with "P"); iFrame.
  Qed.

  Lemma wpsim_assume_tgt (P : iProp Σ) r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    P ∗ wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t tt) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (Assume P) >>= k_t) E.
  Proof. unseal; iIntros "[P RR] I". iApply isim_Assume_tgt; eauto. iFrame. iApply "RR". ss. Qed.

  Lemma wpsim_guarantee_src (P : iProp Σ) r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    P ∗ wpsim r g R_s R_t RR true pt nths (st_s, k_s tt) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger (Guarantee P) >>= k_s) (st_t, i_t) E.
  Proof. unseal; iIntros "[P RR] I". iApply isim_Guarantee_src; eauto. iFrame. iApply "RR"; ss. Qed.

  Lemma wpsim_guarantee_tgt (P : iProp Σ) r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    P ∗ wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t tt) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger (Assume P) >>= k_t) E.
  Proof. unseal; iIntros "[P RR] I". iApply isim_Assume_tgt; eauto. iFrame. iApply "RR". ss. Qed.

  Lemma wpsim_spawn r g R_s R_t RR ps pt nths st_s st_t fn args k_s k_t E :
    wpsim r g R_s R_t RR true true (S nths) (st_s, k_s nths) (st_t, k_t nths) E ⊢
    wpsim r g R_s R_t RR ps pt nths
  ...
  Lemma wpsim_yield r g R_s R_t RR ps pt nths st_s st_t tid k_s k_t E :
    Ist nths st_s st_t ∗
    (∀ nths' st_s' st_t' (NODS : List.NoDup (map fst st_s')) (NODT : List.NoDup (map fst st_t')),
      Ist nths' st_s' st_t' -∗
      wpsim r g R_s R_t RR true true nths' (st_s', k_s tt) (st_t', k_t tt) E) ⊢
    wpsim r g R_s R_t RR ps pt nths
  ...
  Lemma wpsim_tid_src r g R_s R_t RR ps pt nths st_s st_t k_s i_t E :
    wpsim r g R_s R_t RR true pt nths (st_s, k_s my_tid) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, trigger Tid >>= k_s) (st_t, i_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_tid_src; iApply "RR"; iFrame. Qed.

  Lemma wpsim_tid_tgt r g R_s R_t RR ps pt nths st_s st_t i_s k_t E :
    wpsim r g R_s R_t RR ps true nths (st_s, i_s) (st_t, k_t my_tid) E ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, trigger Tid >>= k_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_tid_tgt; iApply "RR"; iFrame. Qed.

  Lemma wpsim_progress r g R_s R_t RR nths st_s st_t i_s i_t E :
    wpsim g g R_s R_t RR false false nths (st_s, i_s) (st_t, i_t) E ⊢
    wpsim r g R_s R_t RR true true nths (st_s, i_s) (st_t, i_t) E.
  Proof. unseal; iIntros "RR I". iApply isim_progress; iApply "RR"; iFrame. Qed.

  Lemma wpsim_base r g R_s R_t RR ps pt nths st_s st_t i_s i_t :
    r R_s R_t RR ps pt nths (st_s, i_s) (st_t, i_t) ⊢
    wpsim r g R_s R_t RR ps pt nths (st_s, i_s) (st_t, i_t) ⊤.
  ...
  Lemma wpsim_coind (r g : rel) A P RA_s RA_t RRA psA ptA nthsA srcA tgtA :
    (∀ (g' : rel) (a : A),
      ...
      wpsim r g' (RA_s a) (RA_t a) (RRA a) (psA a) (ptA a) (nthsA a) (srcA a) (tgtA a) ⊤) →
    ∀ (a : A), P a ⊢
      wpsim r g (RA_s a) (RA_t a) (RRA a) (psA a) (ptA a) (nthsA a) (srcA a) (tgtA a) ⊤.
  Proof.
