...
Require Import PCM.
Require Import Any.
Require Import HoareDef STB ModSim.
...
Section AUX.
  ...
  | is_simple_intro
      (PRE: forall x varg varg_tgt, ⊢ fsp.(precond) x varg varg_tgt -* ⌜varg = varg_tgt⌝)
      (POST: forall x vret vret_tgt, ⊢ fsp.(postcond) x vret vret_tgt -* ⌜vret = vret_tgt⌝)
  .
...
(WF: URA.wf (fmr ⋅ hdr))
  ...
  Lemma harg_clo2
        A
        r rg
        X_src (P_src: X_src -> Any.t -> Any.t -> iProp)
        X_tgt (P_tgt: X_tgt -> Any.t -> Any.t -> iProp)
        arg_tgt
        fl_src fl_tgt
        ...
        (ARG: forall arg_src x_src, exists x_tgt FR,
            <<SEP: bi_entails (R a mp_src mp_tgt ** P_src x_src arg_src arg_tgt)
                              (bupd (P_tgt x_tgt arg_tgt arg_tgt ** FR))>>
                   /\
            <<SIM: forall mr_src' fr_src fr_tgt
                  (ACC: current_iPropL (fr_src ⋅ (mr_tgt ⋅ mr_src'))
                                       [("FR", FR); ("TF", OwnT fr_tgt); ("TM", OwnT mr_tgt)]),
                gpaco8 (_sim_itree (mk_wf R) le fl_src fl_tgt) (cpn8 (_sim_itree (mk_wf R) le fl_src fl_tgt)) r rg _ _ eqr true true a
                       (Any.pair mp_src (mr_tgt ⋅ mr_src')↑, k_src (fr_src, (x_src, arg_src)))
                       (Any.pair mp_tgt mr_tgt↑, k_tgt (fr_tgt, (x_tgt, arg_tgt)))>>)
    :
      gpaco8 (_sim_itree (mk_wf R) le fl_src fl_tgt) (cpn8 (_sim_itree (mk_wf R) le fl_src fl_tgt)) r rg _ _ eqr m n a
             ((Any.pair mp_src mr_src↑), (HoareFunArg P_src arg_tgt >>= k_src))
             ((Any.pair mp_tgt mr_tgt↑), (HoareFunArg P_tgt arg_tgt >>= k_tgt))
  .
  Proof.
        ...
        (POST: forall x_tgt (PURE: ord_lt (fsp_tgt.(measure) x_tgt) o_tgt
          ...
          exists I FR,
            ...
            (<<UPDATABLE: bi_entails I (OwnT (fr_tgt0) ** OwnT (mr_tgt0) **
                              (fsp_tgt.(precond) x_tgt arg_tgt arg_tgt
                                 ==∗ (FR ** (∃ a1, R a1 mp_src0 mp_tgt0 ** ⌜le a0 a1⌝) **
                                         (fsp_src.(precond) x_src arg_src arg_tgt: iProp))))>>) /\
            (<<POST: forall (ret_src ret_tgt: Any.t) (mp_src1 mp_tgt1 : Any.t),
            exists J,
              (<<UPDATABLE: (FR ** (∃ a1, R a1 mp_src1 mp_tgt1 ** ⌜le a0 a1⌝) **
                                          fsp_src.(postcond) x_src ret_src ret_tgt) ==∗
                            (fsp_tgt.(postcond) x_tgt ret_tgt ret_tgt ** J)>>) /\
    ...
    :
      gpaco8 (_sim_itree (mk_wf R) le fl_src fl_tgt) (cpn8 (_sim_itree (mk_wf R) le fl_src fl_tgt)) r rg _ _ eqr m n a
             (Any.pair mp_src0 (mr_tgt0 ⋅ mr_src0)↑,
                       (HoareCall tbr_src o_src fsp_src fn arg_src) fr_src0 >>= k_src)
             (Any.pair mp_tgt0 mr_tgt0↑,
                       (HoareCall tbr_tgt o_tgt fsp_tgt fn arg_tgt) fr_tgt0 >>= k_tgt)
  .
  Proof.
    ...
    steps_safe.
    repeat (ired_both; apply sim_itreeC_spec; econs 8). exists (ro_src, (fr_src ⋅ fr_tgt), (mr_tgt1 ⋅ mr_src1)).
    repeat (ired_both; apply sim_itreeC_spec; econs 12). s. rewrite Any.pair_split.
    repeat (ired_both; apply sim_itreeC_spec; econs 8). unshelve esplits.
    ...
    repeat (ired_both; apply sim_itreeC_spec; econs 12). s. 
    repeat (ired_both; apply sim_itreeC_spec; econs 8). esplits; et.
    repeat (ired_both; apply sim_itreeC_spec; econs 8). esplits; et.
    repeat (ired_both; apply sim_itreeC_spec; econs 8). unshelve esplits; et.
    repeat (ired_both; apply sim_itreeC_spec; econs 8). unshelve esplits; et.
  ...
  Lemma hret_clo2
        ...
        (eqr: Any.t -> Any.t -> Any.t -> Any.t -> Prop)
        (R: A -> Any.t -> Any.t -> iProp)
        (SIMPL: forall x vret vret_tgt, ⊢ Qt x vret vret_tgt -* ⌜vret = vret_tgt⌝)
        ...
        (UPDATABLE:
          bi_entails I (OwnT (fr_tgt) ** OwnT (mr_tgt) **
                             (Qt xt vret_tgt vret_tgt
                                 ==∗ ((∃ a, R a mp_src mp_tgt ** ⌜le a0 a⌝)
                                        ** (Qs xs vret_src vret_tgt: iProp)))))
    ...
    :
      gpaco8 (_sim_itree (mk_wf R) le fl_src fl_tgt) (cpn8 (_sim_itree (mk_wf R) le fl_src fl_tgt)) r rg _ _ eqr m n a0
             (Any.pair mp_src (mr_tgt ⋅ mr_src)↑, (HoareFunRet Qs xs (fr_src, vret_src)))
             (Any.pair mp_tgt mr_tgt↑, (HoareFunRet Qt xt (fr_tgt, vret_tgt)))
  .
  Proof.
    ...
    forall fn fsp_tgt (FIND: stb_tgt fn = Some fsp_tgt) x_tgt (PURE: is_pure (fsp_tgt.(measure) x_tgt)),
      exists fsp_src x_src, (<<FIND: stb_src fn = Some fsp_src>>) /\
                              ...
                              (<<PRE: forall arg_src arg_tgt,
                                  fsp_src.(precond) x_src arg_src arg_tgt ==∗
                                                    fsp_tgt.(precond) x_tgt arg_src arg_tgt>>) /\
                              (<<POST: forall ret_src ret_tgt,
                                  fsp_tgt.(postcond) x_tgt ret_src ret_tgt ==∗
                                                    fsp_src.(postcond) x_src ret_src ret_tgt>>)
  .
...
(SIMPL: forall fn fsp_tgt (IN: stb_tgt fn = Some fsp_tgt), is_simple fsp_tgt)
    ...
    :
      gpaco8 (_sim_itree (mk_wf R) le fl_src fl_tgt) (cpn8 (_sim_itree (mk_wf R) le fl_src fl_tgt)) r rg _ _ eqr m n _a
             (Any.pair mp_src0 (mr_tgt0 ⋅ mr_src0)↑, (interp_hCallE_tgt stb_src o_src APC fr_src0) >>= k_src)
             (Any.pair mp_tgt0 mr_tgt0↑, (interp_hCallE_tgt stb_tgt o_tgt APC fr_tgt0) >>= k_tgt)
  .
  Proof.
    subst.
    ...
    clear_until x. gen x o_src o_tgt fr_src0 fr_tgt0. gen mp_src0 mp_tgt0 mr_src0 mr_tgt0. gen a0. gcofix CIH. i.
    {
      ...
      - steps. eapply gpaco8_mon; et.
        ...
        { unfold HoareCall. unfold mput, mget. steps_safe. des. contradict H0. r. eauto. }
