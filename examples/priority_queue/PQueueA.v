...
Require Import StackHeader StackA StackIA.
...
Section definitions.
  ...
  Definition syn_queue_inv
    ...
    (qb, qofs) ↦ Vint range ∗
    [∗ n list] i ↦ x ∈ entries, (qb, qofs + i + 1)%Z ↦ x.1
  )%SAT.
...
Module PQueueA. Section PQueueA.
  ...
  Definition add : Any.t → itree crisE Any.t :=
    atomic_body (from_fspec add_spec)
  ...
  Definition remove_min : Any.t → itree crisE Any.t :=
    atomic_body (from_fspec remove_min_spec)
      (λ '(_, _, (γq, range)) _,
        𝒴;;;
  ...
  Definition fnsems : fnsems_type :=
    [(Some PQueueHdr.new, (true, wmask_all, scopes, (Some new_spec, new)));
     (Some PQueueHdr.add, (true, wmask_all, scopes, (None, add)));
     (Some PQueueHdr.remove_min, (true, wmask_all, scopes, (None, remove_min)))].
...
Module PQueueIA. Section PQueueIA.
  ...
  Proof.
    ...
    rewrite repeat_app /=; iDestruct "↦queue" as "[↦range ↦queue]".
    iApply (wsim_mem_store with "[↦range]");
        [try prove_inline_cond|try prove_sb_cond|..|unfold_cris_defs]; iFrame.
    iIntros "↦range". steps_r. hss_r. steps_r.
    ...
    { rewrite ?Nat2Z.id. erewrite <-(bind_ret_r (SB.sandbox _ _ _ _)).
      ...
      { iExists (repeat (Vundef, 1%positive) range).
        ...
        rewrite -Nat.add_1_r ?repeat_app /= ?big_sepL_app /=; case_decide; try lia.
        rewrite ?repeat_length /=.
        iDestruct "↦queue" as "[↦queues [↦queue _]]".
        iPoseProof ("IHrange" with "[↦queues]") as "↦queues"; eauto. iFrame.
        rewrite Nat.add_0_r Z.add_1_r Nat2Z.inj_succ; iFrame.
      }
      set (var := range).
      rewrite {1 2 3 4}/var.
      replace (0%Z) with (range - var)%Z by lia.
      rewrite {1 6}Z.sub_diag.
      ...
      { unfold_iter_r. steps_r.
        erewrite <-(bind_ret_r (SB.sandbox _ _ _ _)). steps_l.
        sch_yield_ir. steps_r.
        sch_yield_l. ss.
        ...
        iApply wsim_unfold; iIntros "W". step. iFrame "IST TID W". iExists γq. iSplitR.
        { iExists _, _; iSplit; first done.
          iExists entries; iSplit; first done. iSplit; done.
        }
        iExists _; iFrame; ss. rewrite repeat_length; iSplit; first done.
      ...
      { apply sp_incl_to_sp; rewrite /SchA.sp; unseal CRIS; ss; prove_nodup. }
      { eapply union_difference, nclose_subseteq. }
      { set_solver. }
      steps_r. iDestruct "GRT" as "[TID [[%stack [%γs [-> [#is_stack stack]]]] %]]".
      hss_r. steps_r.
      rewrite ?copset_diff_union; try eapply nclose_subseteq.
      sch_yield_ir. steps_r.
  ...
  Lemma add_simF : ISim.sim_fun open PQueueA PQueueI init_cond IstFull (Some PQueueHdr.add).
  Proof.
    init_simF.
    steps_l. iDestruct "ASM" as "[TID [[%n [%q [[-> %] #[%queueb [%queueofs [-> is_queue]]]]]] _]]".
    rename _q2 into stid, _q3 into mtid, _q5 into v, _q7 into priority, _q8 into γq, _q9 into range.

    steps_r. hss_r. steps_r. sch_yield_ir. steps_r. sch_yield_ir. steps_r.
    iDestruct "is_queue" as "[%entries [%Hlen [#qinv #stacks]]]".
    iInv "qinv" as "inv" "close". rewrite SLRed_eq.
    iDestruct "inv" as "[◯entries [↦range ↦]]".
    ...
    { iFrame. iApply ("↦s" with "[] [↦]"); iFrame.
      iIntros "!> %%%% [?$]".
    }
    sch_yield_ir. steps_r.
    
    (* stack push *)
    inline_r. steps_r. force_r (_, _, (n, stack, v, γs)); forces_r. iFrame. iSplit; eauto. steps_r.
    sch_yield_ii.
    { apply sp_incl_to_sp; rewrite /SchA.sp; unseal CRIS; ss; prove_nodup. }
    { eapply union_difference, nclose_subseteq. }
    { set_solver. }
    ...
    (* atomic update *)
    sch_yield_l. steps_l. rename _q into queue. set (entry := queue !!! priority).
    iDestruct "ASM" as "[%entries' [%Hlen [● stack_contents]]]".
    iInv "qinv" as "inv" "close". rewrite SLRed_eq.
    iDestruct "inv" as "[◯ inv]". iCombine "●" "◯" gives %->%excl_auth_agree_L.
    ...
    rewrite ?copset_diff_union; cycle 1.
    { apply nclose_subseteq. }
    iDestruct "GRT" as "[TID _]".
    sch_yield_ir.
    sch_yield_l. steps_l. force_l. iFrame. iSplit; eauto.
    step. iFrame. done.
    Unshelve. all: eauto.
  (*SLOW*)Qed.

  Lemma remove_min_simF :
    ISim.sim_fun open PQueueA PQueueI init_cond IstFull (Some PQueueHdr.remove_min).
  Proof.
    init_simF.
    steps_l. iDestruct "ASM" as "[TID [[%n [%q [-> #[%queueb [%queueofs [-> Q]]]]]] _]]".
    rename _q2 into stid, _q3 into mtid, _q4 into γq, _q5 into range.

    steps_r. hss_r. steps_r. sch_yield_ir. steps_r. sch_yield_ir. steps_r.
    iDestruct "Q" as "[%entries [%Hlen [#queue_inv #stack_invs]]]".
    iInv "queue_inv" as "inv" "close"; rewrite SLRed_eq.
    iDestruct "inv" as "[◯ [↦range ↦queues]]".

    (* range load *)
    iApply (wsim_mem_load with "[↦range]");
      [try prove_inline_cond|try prove_sb_cond|ss|unfold_cris_defs].
    iIntros "↦range". steps_r. hss_r. steps_r.
    iMod ("close" with "[◯ ↦range ↦queues]") as "_"; iFrame.
    sch_yield_ir. steps_r. sch_yield_l. steps_l.
    rewrite Nat2Z.id. set (var := length entries). rewrite {1 2 3 4 6}/var.
    iAssert (⌜var ≤ length entries⌝)%I as "#Hvar"; first done.
    replace (queueofs + 1)%Z with (queueofs + (length entries - var) + 1)%Z by lia.
    generalize var; clear var; iIntros (var).
    iInduction (var) as [|var'] forall (st_src st_tgt).
    { unfold_iter_l; unfold_iter_r. steps_l; steps_r.
      sch_yield_ir. steps_r. sch_yield_l. steps_l. force_l. iFrame; iSplit; eauto.
      step; iFrame; done.
    }
    ...
    iApply (wsim_mem_load with "[↦queue]");
      [try prove_inline_cond|try prove_sb_cond|ss|unfold_cris_defs].
    iIntros "↦queue". steps_r. hss_r. steps_r.
    ...
    inline_r. steps_r. force_r (stid, mtid, (n, istack, iγs)). forces_r. iFrame. iSplit; eauto.
    steps_r. sch_yield_ii.
    { apply sp_incl_to_sp; rewrite /SchA.sp; unseal CRIS; ss; prove_nodup. }
    { eapply union_difference, nclose_subseteq. }
    { set_solver. }
    steps_r.
    ...
    { set_solver. }
    steps_r. hss_r. steps_r.
    rewrite ?copset_diff_union; try apply nclose_subseteq.
    ...
    { 
      ...
      steps_r. iDestruct "GRT" as "[TID _]". sch_yield_ir. steps_r.
  ...
  Lemma sim :
    ISim.t open PQueueA PQueueI emp IstFull.
  Proof.
    init_sim.
    { split; ss. iIntros "_"; iSplit; ss. }
    { apply new_simF. }
    { apply add_simF. }
