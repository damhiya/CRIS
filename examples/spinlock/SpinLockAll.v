...
Module IncrAll.
  ...
  Local Instance Γ : HRA := ##[invΓ; memΓ; SpinLockΓ; SpinLockMainAΓ].
  Local Instance Σ : GRA := ##[invΣ; SchAΣ; Γ].

  Local Definition spc_user_s : univ_id → string → option fspec :=
    λ u, to_spc (SpinLockMainAS.spc u ++ MemA.Spc).

  Local Definition smod_src : univ_id → SMod.t :=
    λ u, (SpinLockMainA.Mod u) ☆ (MemA.Mod) ☆ (SchA.Mod u (spc_user_s u)).
  ...
  Local Definition mod_src : univ_id → HMod.t :=
    λ u, SMod.to_hmod (wsim_ginv u ⊤) (spc_s u) (smod_src u).

  Local Definition csl : string → bool := λ _, false.
  Local Definition genv : GEnv.t := GEnv.unit.
(* 
  Local Definition mod_tgt : HMod.t := SpinLockMainI.t ★ (MemI.t csl genv) ★ (SchI.t).
